...
// Загрузка данных с AirVisual API
async function loadAirVisualData() {
    for (const district of districts) {
        ...
        for (const point of district.points) {
            try {
                const airData = await fetchAirVisualData(point.lat, point.lon);
                if (airData && airData.current && airData.current.pollution) {
                    totalUS_AQI += airData.current.pollution.aqius;
                    validStations++;
                }
            } catch (error) {
                console.warn(`Нет данных для ${district.name}:`, error);
            }
        }

        const avgAQI = validStations > 0 ? Math.round(totalUS_AQI / validStations) : 50;
        ...
        createDistrictPolygon(district, avgAQI);
    }
}

// Запрос к API
async function fetchAirVisualData(lat, lon) {
    ...
    try {
        ...
        console.log('🌐 AirVisual API недоступен, используем демо-данные');
    }
...
function createDemoData() {
    ...
    districts.forEach(district => {
        const baseAQI = 20 + Math.random() * 120;
        const aqius = Math.round(baseAQI);
